...
struct snode {         //////// VARIABLES
	...
	friend ll get_sum(sn x) { return x ? x->sum : 0LL; }
    void prop() {  // lazy prop
		if (!flip) return;
		swap(c[0], c[1]);
		flip = 0;
	...
	void upd() {  // recalc vals
        ...
        sum = get_sum(c[0]) + get_sum(c[1]) + val;
	}
	void vupd(){}
	...
	//////// MODIFICATIONS
	void set(long long v) { access(); val = v; upd(); } // changes value
	friend void link(sn x, sn y, bool force = 0) { // x ose lesz y-nak | ha force: x - y él minden esetben | ha nem force: akkor y-nak gyökérnek kell lenni
...
Út querry-hez a get_par-hoz hasonló implementáció kell + fbo implementáció, ha nem a teljes út kell. | Ha a, b út kell: 1. make_root(a), 2. query b-tol gyökérig | lca-hoz hasonlóan a->access(), b->access(), a->c[0] lca-ig vezeto út
...
int main(){
    ...
    for(int i = 0; i < q; i++){
        ...
        if(t == 0){
            ...
            int a, x;
            cin>>a>>x;
            lct[a]->set(lct[a]->val + x);
        } else{
            int a, b;
            cin>>a>>b;
            lct[a]->make_root();
            lct[b]->access();
            cout << (get_sum(lct[b]->c[0]) + lct[b]->val) << '\n';
        }
    }
